...
double my_cbrt_2(double n)   // more accurate cube root
{
	double r = 1;
	while (n>1 || n<(1/8)) {
		if (n>1) {
...
int main()
{
	try {
		...
		print_element("my_cbrt_2", col_width);
		...
		for (auto k : {-100,-10,-1,0,1,10,100}) {    // n = pi*10^k,k={-100,-10,-1,0,1,10,100}
			...
			print_element(my_cbrt_2(n), col_width);
			print_element(100*((my_cbrt_2(n)-cbrt(n))/cbrt(n)), col_width);
			cout << '\n';
		}
	}
	catch (exception& e) {
